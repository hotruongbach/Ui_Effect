    ...
    {        
        ...
        private void PrepareLayersGroup()
        {
            ...
            for (int i = 0; i < this.pyxelTilemap.layers.Length; i++)
            {
                ...
                Tilemap map = goLayer.GetComponent<Tilemap>();
                if (map == null) map = goLayer.AddComponent<Tilemap>();
        ...
        private void DrawTileToLayers()
        {
            ...
            for (int i = 0; i < this.pyxelTilemap.layers.Length; i++)
            {
                Tilemap map = pyxelEdit.transform.Find(this.pyxelTilemap.layers[i].name).GetComponent<Tilemap>();
